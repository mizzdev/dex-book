...
describe('Integration test suite: 20190227_market_buy_vlimit_8digits', () => {
  ...
  test('Market order should be accepted and processed by the book', () => {
    ...
    orderProcessingResult = book.addOrder(new MarketOrder({
      ...
      volumeLimitQtyPrecision: precision,
      side: 'BUY'
    }));
  ...
  test('Two trades should occur and "trade" events should be emitted', () => {
    ...
    expect(String(listener.mock.calls[1][0].qty)).toBe('4.83870967');
  });
  ...
  test('Four order fills should occur and "order-filled" events should be emitted', () => {
    ...
    expect(String(listener.mock.calls[2][0].qty)).toBe('4.83870967');
    ...
    expect(String(listener.mock.calls[3][0].qty)).toBe('4.83870967');
