import { Book, LimitOrder, StopLimitOrder } from '../../../src';

describe('Integration test suite: 20190303_stop_limit_sell', () => {
  ...
  test('Bid, ask and market prices should change correctly during the test', () => {
    ...
    expect(String(book.bid)).toBe('undefined');
    expect(String(book.ask)).toBe('0.8');
  ...
  test('A trade should occur and "trade" event should be emitted', () => {
    ...
    expect(String(listener.mock.calls[0][0].price)).toBe('2');
    ...
    expect(listener.mock.calls[2][0].oppositeOrder.id).toBe(order2Id);
    expect(String(listener.mock.calls[2][0].price)).toBe('1');
  ...
  test('Two order fills should occur and "order-filled" events should be emitted', () => {
    ...
    expect(listener.mock.calls[3][0].order.id).toBe(order2Id);
    expect(listener.mock.calls[3][0].oppositeOrder.id).toBe(order4Id);
    expect(String(listener.mock.calls[3][0].price)).toBe('1');
    ...
    expect(listener.mock.calls[4][0].oppositeOrder.id).toBe(order2Id);
    expect(String(listener.mock.calls[4][0].price)).toBe('1');
    expect(String(listener.mock.calls[4][0].qty)).toBe('1');
    expect(listener.mock.calls[4][0].isFull).toBe(false);

    expect(listener.mock.calls[5][0].order.id).toBe(order2Id);
    expect(listener.mock.calls[5][0].oppositeOrder.id).toBe(order3Id);
    expect(String(listener.mock.calls[5][0].price)).toBe('1');
