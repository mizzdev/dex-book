...
describe('Integration test suite: 20190303_limit_id_conflict', () => {
  ...
  test('Conflicting order should be rejected', () => {
    ...
    expect(orderProcessingResult.errorCode).toBe(EBookErrorCode.ID_CONFLICT);
  });
  ...
  test('"order-rejected" event should be emitted', () => {
    ...
    expect(listener.mock.calls[0][1]).toBe(EBookErrorCode.ID_CONFLICT);
  });
});
